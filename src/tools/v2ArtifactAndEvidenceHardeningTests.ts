...
test("HARDENING-4 & HARDENING-14: Artifact Deletion/Tampering Detected by ProMax & Rejected by Lab", () => {
  ...
  try {
    ...
    const resMod = kernel.runProMaxVerification(candidate, context, [], verifier);
    ...
    const resDel = kernel.runProMaxVerification(candidate, context, [], verifier);
...
test("HARDENING-5: Stale Evidence Causality Rejection across Missions", () => {
  ...
  try {
    ...
    const res = kernel.runProMaxVerification(candidate, context, [foreignEvidence], verifier);
