...
test("ProMaxVerifier: Generates Proof Mappings and Verifies Observed Evidence", () => {
  ...
  try {
    ...
    const result = kernel.runProMaxVerification(candidate, context, [cmdRes.evidenceRecord!, acEv], verifier);
...
test("ProMaxVerifier: P0.17 Stale Evidence Fails Verification and Leaves Criteria Unverified", () => {
  ...
  try {
    ...
    const result = kernel.runProMaxVerification(candidate, context, staleEvidence, verifier);
