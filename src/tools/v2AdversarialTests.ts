...
test("P0-RA8 8-Point Cross-Verifier Permit Confusion & Boundary Matrix", () => {
  ...
  try {
    ...
    const cand: IntegratedCandidate = {
      candidateId: "cand-ra8",
      missionId: "mission-A",
      integratedArtifacts: [{ artifactId: "a1", path: "src/index.ts", sha256: createHash("sha256").update("export const x = 1;").digest("hex"), sizeBytes: 19, missionId: "mission-A" }],
      resolvedConflicts: [],
      sourceTraceability: { "src/index.ts": "COLONY_A" },
...
test("P0-RA6 & P0-RA7 Execution Receipt Authenticity, Permit Confinement & Malicious Verifier Attack Matrix", () => {
  ...
  try {
    ...
    // 2. Direct calls to emitTestQualificationProof without kernel-minted active permit throw UNAUTHORIZED_VERIFIER_PERMIT
    assert.throws(
      () => kernel.emitTestQualificationProof(fakePermit, "m-ra", "PROMAX", { criterionId: "ac-1" }),
      /UNAUTHORIZED_VERIFIER_PERMIT/
    );
    ...
    const candMalicious: IntegratedCandidate = {
      candidateId: "cand-mal",
      missionId: "m-ra",
      integratedArtifacts: [],
      resolvedConflicts: [],
      sourceTraceability: {},
      workspacePath: "workspaces/v2-missions/m-ra/leggo-integrated",
    };
    const ctxMalicious: ContractBoundStageContext = {
      missionId: "m-ra",
      authoritativeInputs: [],
      policyVersions: ["v1.0.0"],
      budgets: { virtualTicks: 100, providerCalls: 10, maxFixAttempts: 3 },
