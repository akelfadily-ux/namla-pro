...
/**
 * The result of ONE bounded container-enumeration query. Structurally a
 * `spawnSync` return, so the real value is passed straight in with no adapter.
 */
export interface ContainerEnumerationOutcome {
...
/**
 * The fixed, bounded enumeration argv. No shell, no caller-chosen flag, and
 * deliberately NO `--filter name=`.
 *
 * A name filter looked like a cheap way to bound the output, but it would have
 * put Docker's matching language inside the security proof: the filter is
 * documented as matching all or PART of a name, and Moby evaluates the value as
 * a REGULAR EXPRESSION after an exact-match fast path. The proof would then rest
 * on those semantics before our own exact comparison ever ran. Enumerating
 * everything and comparing exactly depends on nothing but "the query succeeded"
 * and "this exact string is or is not in the answer".
 */
export function containerEnumerationArgs(): readonly string[] {
  return ["container", "ls", "--all", "--format", "{{json .Names}}"];
}

/**
 * Is the container PROVEN absent by a query that actually SUCCEEDED (S-16)?
 *
 * Two successive versions of this decision were wrong, and both failures were
 * the same mistake at different depths.
 *
 *   `check.status !== 0`        `spawnSync`'s `status` is `null` whenever the
 *                               process produced no exit code at all — missing
 *                               binary, timeout, `maxBuffer` overrun, signal.
 *                               `null !== 0` is `true`, so "we could not look"
 *                               read as "we looked and it was gone".
 *
 *   completed non-zero exit     Excluding the `null` cases still left this, and
 *                               it is also false. Docker's CLI collects any
 *                               error from the object lookup and returns
 ...
 *                               A non-zero status proves "inspect failed", never
 *                               "the container does not exist". The predicate
 *                               read exactly `(error, status)`, so two different
 *                               realities produced the identical input — the
 *                               collision was structural, not a matter of
 *                               enumerating more exit codes.
 *
 * So absence is no longer inferred from a FAILURE at all. It is established
 * only by an AFFIRMATIVE, SUCCESSFUL enumeration: the query ran, exited 0,
 * returned structurally valid machine-parseable output, and that output does not
 * contain the exact target name. Everything else is UNKNOWN, and unknown is
 * never removed — the same discipline `processTree.ts` applies to its process
 * snapshot ("unresolvable tool, spawn failure, timeout, buffer overrun, or
 * non-zero exit: no evidence").
 *
 * Error TEXT is deliberately never consulted. "No such object" is a
 * human-readable message — localizable, version-dependent, and influencable
 * through a container name. A message is not a security protocol.
 */
export function containerAbsenceProven(targetName: string, query: ContainerEnumerationOutcome): boolean {
  // A name is required to compare against, and nothing more is demanded of it.
  //
  // An earlier draft required a `namla-(run|verify|bisect)-…` grammar here. That
  // was wrong twice over: it encoded a NAMING POLICY into a generic evidence
  // parser, and it would have rejected a name the exported
  // `runStageBisection(runner, inputs, namePrefix)` API can legitimately
  // generate. Its stated justification was also unsound once the filter was
  // removed — `targetName` no longer reaches a Docker filter, a regex, a shell,
  // or a command template. The enumeration argv below is identical whatever the
  // target is, so `.`, `*`, `^`, `$` and `|` are ordinary characters in an
  // exact JavaScript string comparison and can alter nothing.
  if (typeof targetName !== "string" || targetName.length === 0) return false;

  // The call itself failed: nothing it "reported" is evidence of anything.
  ...
  if (typeof query.status !== "number") return false;
  // THE CORRECTION: the enumeration must have SUCCEEDED. A failed query says
  // only that the query failed, never that the container is gone.
  if (query.status !== 0) return false;
  // Output must be present to be an answer. Missing output is not an empty list.
  if (typeof query.stdout !== "string") return false;

  // Every line must be a JSON-encoded name. One unparseable line makes the whole
  // listing unknown: a parser that skipped it could skip the line naming the
  // target, turning a truncated answer into a false proof of absence.
  for (const line of query.stdout.split("\n")) {
    ...
    // Docker reports several names for one container as a comma-joined field.
    // Compare each EXACTLY: `--filter name=` is a substring match, so genuine
    // near-misses come back and must not be mistaken for the target.
    for (const name of value.split(",")) {
      if (name.trim() === targetName) return false; // enumerated: still present
    }
  }

  // The runtime answered, the answer parsed, and the target is not in it.
  return true;
}
...
export class DockerContainerSandboxBackend implements ContainerSandboxBackend {
  ...
  private forceRemove(runtime: string, name: string): boolean {
    ...
    const query = spawnSync(runtime, [...containerEnumerationArgs()], { shell: false, encoding: "utf8", timeout: PROBE_HELPER_TIMEOUT_MS, killSignal: PROBE_KILL_SIGNAL, maxBuffer: 262144, windowsHide: true });
    return containerAbsenceProven(name, query);
  }
