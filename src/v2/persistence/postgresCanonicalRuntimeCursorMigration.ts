...
} from "./postgresCanonicalRuntimeCursorSchema";

export const V2_POSTGRES_CANONICAL_RUNTIME_MIGRATION_VERSION =
  3 as const;

export const V2_POSTGRES_CANONICAL_RUNTIME_MIGRATION_NAME =
  "v2-canonical-runtime-checkpoint-v1" as const;

export type V2PostgresCanonicalRuntimeMigrationResult =
  | "APPLIED"
  | "ALREADY_APPLIED";
...
function mapMigrationRows(
  ...
  for (const row of rows) {
    ...
    if (
      ...
      throw new Error(
        "POSTGRES_CANONICAL_RUNTIME_INVALID_MIGRATION_NAME",
      );
    }
...
/**
 * Applies canonical runtime checkpoint persistence as V2 migration 3.
 *
 * Migration order is strict:
 *
 * 1. mission checkpoint persistence
 * 2. execution authority
 * 3. canonical runtime checkpoint persistence
 *
 * A missing or renamed prerequisite fails closed.
 */
export async function migrateV2CanonicalRuntimeCursorSchema(
  database:
    PostgresCheckpointDatabase,
): Promise<V2PostgresCanonicalRuntimeMigrationResult> {
  return database.transaction(
    async (client) => {
      /*
       * Must remain the first database operation.
       *
       * The complete V2 migration family shares one transaction-scoped
       * advisory lock so independent processes cannot race the ledger.
       */
      await client.query(
        `
