...
| Verifier Identity Impersonation | **PUBLIC_QUALIFICATION_PROOF_DOWNGRADE** (P0-RA1, P0-RA3) | Public `TrustedKernel.emitEvidence` strictly CANNOT mint `QUALIFICATION_PROOF` under any producer name (mandatory downgrade to `TRACEABILITY` or `CLAIM`). Authentic qualification proofs are minted exclusively via dedicated internal methods (`emitVerifierQualificationProof`, `emitSecurityQualificationProof`). |
