...
- **Aggregate Reconciliation Ledger & Specific Reservation Reconciliation:** Updated multi-reservation recovery in `PostgresStateRepository.recoverAccountingState` to use aggregate reconciliation ledger rows or fail-closed specific reservation recovery, maintaining BLOCKED accounting state until all uncertainty is resolved.
